...
class TaxGroupComponent extends Component {
    setup() {
        ...
        onPatched(() => {
            if (this.state.value === "edit") {
                ...
                const newVal = taxGroup.tax_group_amount.toFixed(3);
    ...
    _onChangeTaxValue() {
        ...
        const oldValue = this.props.taxGroup.tax_group_amount.toFixed(3);
        let newValue;
        try {
...
export class TaxTotalsComponent extends Component {
    ...
    formatData(props) {
        ...
        for (let subtotal_title of totals.subtotals_order) {
            ...
            for (let i in group) {
                amount_tax = (amount_tax + group[i].tax_group_amount).toFixed(2);
            }
        }
